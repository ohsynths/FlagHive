...
@section('title', "FlagHive - {$user->name}")
...
@section('styles')
    ...
    .writeup-row:last-child { border-bottom: 1px solid #333; }

    .writeup-row:hover { background-color: #0a0a0a; }

    .writeup-row a { color: #fff; text-decoration: none; }
    .writeup-row a:hover { text-decoration: underline; }
    ...
    .writeup-row .meta .badge { color: #888; }
    ...
    .empty-card p { font-size: 0.875rem; color: #888; }
...
@section('content')
    <main>
        <div class="card">
            <p class="card-header"><span class="dollar">$</span> cat /home/{{ $user->name }}/profile</p>
        ...
        <h2 class="section-title">Writeups by {{ $user->name }}</h2>
        ...
        @if ($writeups->hasPages())
            <div style="margin-top:1.5rem">{{ $writeups->links() }}</div>
        @endif
    </main>
@endsection
